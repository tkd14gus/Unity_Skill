...
public class Turet : MonoBehaviour
{
    ...
    Transform m_tfTarger = null;

    void SearchEnemy()
    {
        Collider[] t_cols = Physics.OverlapSphere(transform.position, m_range, m_layerMask);
        Transform t_shortesTarget = null;

        if(t_cols.Length > 0)
        {
            float t_shortesDistance = Mathf.Infinity;
            foreach(Collider t_colTarget in t_cols)
            {
                ...
                if(t_shortesDistance > t_distance)
                {
                    t_shortesDistance = t_distance;
                    t_shortesTarget = t_colTarget.transform;
                }
            }
        }
        m_tfTarger = t_shortesTarget;
    }
    ...
    void Update()
    {
        ...
        {
            Quaternion t_lookRotation = Quaternion.LookRotation(m_tfTarger.position);
