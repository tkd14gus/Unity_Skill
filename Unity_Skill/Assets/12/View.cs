...
public class View : MonoBehaviour
{
    ...
    [SerializeField] float m_disrance = 0f;
    [SerializeField] LayerMask m_layerMask = 0;

    void Sight()
    {
        Collider[] t_cols = Physics.OverlapSphere(transform.position, m_disrance, m_layerMask);
        ...
        if(t_cols.Length > 0)
        {
            ...
            {
                if(Physics.Raycast(transform.position, t_direction, out RaycastHit t_hit, m_disrance))
                {
